...
int curN, curK;
int curUsableN;
int selected[MAX_LEVEL];
int minSelected[MAX_LEVEL];
...
int main(void) {
    ...
    FOR(tt, t) {
        ...
        if (resultSize[tt] > 0) {
            ...
            for(int i=resultSize[tt] - 1;i>=0;i--) printf("%d ", indexes[tt][result[tt][i]] + 1);
            puts("");
        }
    }
