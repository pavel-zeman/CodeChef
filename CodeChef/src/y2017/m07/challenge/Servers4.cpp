...
int main(void) {
    ...
    FOR(i, n) {
        servers[i].x = getInt();
        servers[i].y = getInt();
        maxc = max(maxc, servers[i].x);
        maxc = max(maxc, servers[i].y);
        servers[i].minp = EMPTY;
        int k = getInt();
        FOR(j, k) {
            int p = getInt();
            maxp = max(maxp, p);
            servers[i].addCpu(j, p);
        }
        if (k > 0) items.insert(SetItem(i));
    }
    //int tries = 300 * 100000 / q;
    //if (maxp <= 1000 || (n > 100 && q > 1000 && maxp > 1000 && maxc > 3000)) tries = 1;
    //if (maxc <= 3000) tries = 1;
